...
let fleekparser = require('fleek-parser');
...
module.exports = (function () {
  let load = function (file, isInConfigDir) {
    ...
    if (isInConfigDir) {
      ...
      defaultFilePath = path.join(ROOT_CONF, file);
    } else {
    ...
    if (fs.existsSync(defaultFilePath)) {
      config = _.clone(require(defaultFilePath), true);
    }

    // Load cutom file overrides
    if (customFilePath && fs.existsSync(customFilePath)) {
      let customConf = _.clone(require(customFilePath), true);
      config = _.defaultsDeep(customConf || {}, config || {});
    }

    if (!config) {
      if (!/local\./.test(file)) console.warn(clc.yellow('  !Failed to load config: ') + file);
      return false;
    } else {
      return config
    }
  ...
  //
  // swagger reconstruction documentation
  //
  let swaggerDoc = fleekparser.parse(root + '/config/api.json');
  ...
  config.swaggerDoc = fleekparser.parse(process.cwd() + '/config/api.json');
